        ...
        public void Write(StreamWriter writer)
        {
            for (int y = 0; y < subdiv; y++)
            {
                for (int x = 0; x < subdiv; x++)
                {
                    if (cells[x, y] != null)
                    {
                        writer.WriteLine("      <a href=\"{0}/{0}.html\"><span class=\"imagemap\" style=\"left:{1}%; top:{2}%; right:{3}%; bottom:{4}%\" onMouseEnter=\"OnEnterImage(document, '{0}')\" onMouseLeave=\"OnLeaveImage(document, '{0}')\" /></a>",
